...
const demoType = Demo.ColorLerp;
switch (demoType as Demo) {
    case Demo.ColorLerp:
        {
            function color(c1: P.Color, c2: P.Color, interpolationMethod: (c1: P.Color, c2: P.Color, t: number) => P.Color) {
                ...
                text.textContent = interpolationMethod.name;
                text.style.position = "absolute";
                text.style.paddingLeft = "16px";
                text.style.color = "white";
                text.style.textShadow = "black 0 1px 2px";
            ...
            function colorShowcase(c1: P.Color, c2: P.Color) {
                color(c1, c2, P.Color.lerp);
                color(c1, c2, P.Color.slerp);
                color(c1, c2, P.Color.hueLerp);
                color(c1, c2, P.Color.hueSlerp);
            }
            colorShowcase(P.Color.red, P.Color.blue);
            colorShowcase(P.Color.green, P.Color.yellow);
            colorShowcase(P.Color.magenta, P.Color.cyan);
            colorShowcase(P.Color.black, P.Color.white);
            colorShowcase(P.Color.fromHexString("#0799ce"), P.Color.fromHexString("#FFAB51"));
            for (let i = 0; i < 3; ++i) {
    ...
    case Demo.Balls:
        {
            ...
            const ballStartingSpeed = 50;
            const ballRadius = 10;
            const ballSpacing = 10;
            const padding = 10;
            const worldMargin = P.Vec2.multiply(P.Vec2.one, ballRadius + padding);
            const min = P.Vec2.add(P.Input.normalizedToWorld(new P.Vec2(0, 0)), worldMargin);
            const max = P.Vec2.subtract(P.Input.normalizedToWorld(new P.Vec2(1, 1)), worldMargin);
            const bounds = new P.Bounds(P.Input.normalizedToWorld(new P.Vec2(0, 0)), P.Input.normalizedToWorld(new P.Vec2(1, 1)));

            class MyBall extends P.RigidBody {
                escaped = false;
                onUpdate(): void {
                    if (!this.escaped && !bounds.contains(this.globalPosition)) {
                        this.escaped = true;
                        console.log("Escaped!");
                    }
                }
            ...
            for (let x = min.x; x <= max.x; x += 2 * ballRadius + ballSpacing) {
                for (let y = min.y; y <= max.y; y += 2 * ballRadius + ballSpacing) {
                    const ball = P.Game.root.createChild(MyBall);
                    ball.name = "Ball";
                    ball.globalPosition = new P.Vec2(x, y);
                    ball.velocity = P.Vec2.multiply(P.Vec2.fromAngle(Math.random() * 360), ballStartingSpeed);
                    const collider = ball.createChild(P.CircleCollider);
